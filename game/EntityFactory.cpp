...
entt::entity factory::create_ball(const GameContext *game_context, sf::Vector2f position,
                                  game::Direction serve_direction)
{
    auto reg = game_context->get_registry();
...
void factory::create_walls(const GameContext *context)
{
    ...
    create_wall(context, game::Direction::Down, {0, static_cast<float>(video_mode.size.y - up_down_wall_margin)},
                {(float)video_mode.size.x, wall_thickness});

    const int left_right_wall_margin = 0;
    create_wall(context, game::Direction::Left, {left_right_wall_margin, 0},
                {wall_thickness, (float)video_mode.size.y});
    create_wall(context, game::Direction::Right,
                {(float)video_mode.size.x - (left_right_wall_margin + wall_thickness), 0},
                {wall_thickness, (float)video_mode.size.y});
}
...
entt::entity factory::create_text(const GameContext *context, std::string text, sf::Vector2f position,
                                  sf::Vector2f size)
{
    auto reg = context->get_registry();
    auto e = reg->create();
